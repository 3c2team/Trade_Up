...
//		System.out.println("전송받은 메세지 : " + message.getPayload()); // JSON 형식 문자열 전달됨
		...
		if(chatMessage.getType().equals(ChatMessage2.TYPE_INIT)) { 
			...
			if(userSessions.size() > 0 && users.size() > 0) { // 사용자 세션 목록이 하나라도 있을 경우에만 수행
				...
				if(users.containsKey(sender_id) && userSessions.containsKey(users.get(sender_id))) {
					userSessions.remove(users.get(sender_id));
				}
			}
			...
			userSessions.put(session.getId(), new ChatUser(sender_id, session));
			...
			if(rooms.size() > 0) { // 채팅방이 1개라도 존재할 경우
				...
				for(Entry<String, List<String>> room : rooms.entrySet()) {
					...
					if(roomUsers.size() == 2 && roomUsers.contains(sender_id) && roomUsers.contains(receiver_id)) {
			...
			if(needCreateRoom) { // 새로운 채팅방 생성이 필요한 경우
				...
				chatMessage.setRoom_id(UUID.randomUUID().toString()); // 새로 생성
//				chatMessage.setRoomId(session.getId()); // 세션 아이디 재사용
				...
				// -------------------------------------------------
				// 생성된 채팅방 DB 에 저장
				// ChatService - addChatRoom() 메서드 호출
				...
				// 채팅방 시작 정보도 알리도록 타입을 다시 START 로 변경하여 메세지 전송 준비
				chatMessage.setType(ChatMessage2.TYPE_START);
				// -------------------------------------------------
			} else { // 기존 채팅방 입장(열기)
				// 재입장 메세지 설정
//				chatMessage.setMessage("> " + receiver_id + " 님과의 채팅방 재입장 <");
			} 
		...
		if(currentRoomUserList != null) {
			for(String roomUserId : currentRoomUserList) {
				...
				if(!roomUserId.equals(chatMessage.getSender_id())) {
			...
			// ----------------------------------------------------------------------------------
			// ChatService - addMessage() 메서드 호출하여 메세지 DB 에 저장(입장, 퇴장, 대화)
			// => 파라미터 : ChatMessage2 객체
